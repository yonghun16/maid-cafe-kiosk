...
지금 이 프로젝트는 안드로이드 태블릿 기준으로 진행 중이라 당장 해당 없지만,
나중에 아이패드로도 배포할 경우를 대비해 iOS 쪽 동일 기능을 참고용으로
남긴다. 구조는 안드로이드와 완전히 대칭이다 — "매장 규모면 코드 없는
가벼운 방법, 기업 규모면 유료 관리 솔루션"이라는 이 문서의 결론도 그대로
적용된다.

- **Guided Access(Screen Pinning과 동급, 코드 없음)**: 설정 → 손쉬운 사용
  → Guided Access 켜기. 앱 실행 중 홈 버튼(또는 사이드 버튼) 3번 연속
  클릭으로 시작 — 안드로이드보다 옵션이 세밀해서 화면 특정 영역
  비활성화, 특정 하드웨어 버튼 무시 등도 추가로 설정 가능. 빠져나가기는
  다시 3번 클릭 후 Face ID/Touch ID 또는 암호 입력.
- **감독 모드(Supervised) + 단일 앱 모드(Device Owner 완전 잠금과 동급)**:
  Apple Configurator로 기기를 한 번 "감독 상태"로 만들어야 하는데, 이미
  개인이 쓰던 기기라면 공장 초기화가 필요하다 — 안드로이드 Device
  Owner와 동일한 제약. 감독 모드가 되면 Apple Configurator나 MDM에서
  "단일 앱 모드"를 원격으로 걸 수 있어 사용자가 뭘 눌러도 못 빠져나가고,
  앱이 직접 코드로 풀어야 한다. 이것도 대개 직접 만들지 않고 Jamf,
  Mosyle, Hexnode 같은 유료 MDM을 구매해서 쓴다.
