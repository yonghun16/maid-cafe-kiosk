...
updated_at: 2026-09-12T02:08:05.873128Z
---
...
- [ ] **실제 브라우저에서 "알림 켜기" 클릭 → 권한 허용 → 다른 기기/탭에서
      주문 → OS 알림이 실제로 뜨는지 확인** — 이번 세션에서는 진행하지
      못함. 에뮬레이터 Chrome을 쓰다가 실제 사용자 Google 계정 이메일이
      로그인 화면에 자동완성되는 걸 발견해 그 즉시 중단한 사고가 있어서
      (관련 기록: 세션 로그 참고), 이후 안전을 위해 브라우저를 직접
      조작하는 검증을 하지 않았다. 알림 권한 허용 자체도 사람이 직접
      눌러야 하는 단계라 자동화하기 어렵다. **사용자가 직접
      `pnpm --filter frontend dev`로는 서비스워커가 꺼져 있으니
      `pnpm --filter frontend build && pnpm --filter frontend start`로
      실행한 뒤 `/kitchen`에서 확인해야 한다.**
