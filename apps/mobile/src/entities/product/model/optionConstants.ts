 ...
 * `'BOTH'`인 메뉴에서만 버튼으로 노출됩니다. 가격에는 영향이 없습니다.
 */
 ...
 * 영향이 없고, HOT을 고르거나 온도를 선택하지 않으면 노출되지 않습니다.
 */
export const ICE_AMOUNT_OPTIONS = ['적게', '적당', '많이'] as const;
export type IceAmount = (typeof ICE_AMOUNT_OPTIONS)[number];

/**
 * "마법의 주문" 옵션에서 고를 수 있는 항목들. 메이드 카페 컨셉의 재미
 * 요소로, 실제 구성 변화나 추가금은 없고 직원이 서빙할 때 외쳐주는
 * 주문을 고객이 고르는 용도입니다. `product.hasMagicSpellOption`이
 * 켜진 메뉴에서만 노출됩니다.
 */
