...
export function OrderCompleteScreen({ order, onDismiss }: OrderCompleteScreenProps) {
  ...
  return (
    <View className="flex-1 items-center justify-center bg-pink-50 p-4">
      <View className="w-full max-w-md rounded-3xl bg-white p-8">
        <Text className="text-center text-4xl">🎀</Text>
        <Text className="mt-2 text-center text-2xl font-bold text-gray-800">주문이 완료됐어요!</Text>
        <Text className="mt-1 text-center text-sm text-gray-500">{ORDER_TYPE_LABEL[order.orderType]}</Text>

        <View className="mt-6 items-center rounded-2xl bg-pink-50 py-6">
          <Text className="text-sm font-medium text-pink-500">주문번호</Text>
          <Text className="mt-1 text-5xl font-extrabold text-pink-500">No. {order.orderNumber}</Text>
        </View>
        ...
        <Pressable onPress={onDismiss} className="mt-8 items-center rounded-xl bg-pink-500 py-3">
